...
@export var max_active_particles: int = 60
@export var particle_skip_ratio_when_busy: float = 0.5
...
func _get_event_bus() -> Node:
	...
	if tree == null:
		return null
	_cached_event_bus = tree.root.get_node_or_null("EventBus")
	return _cached_event_bus
...
func _get_data_manager() -> Node:
	...
	if tree == null:
		return null
	_cached_data_manager = tree.root.get_node_or_null("DataManager")
	return _cached_data_manager
...
func _get_object_pool() -> Node:
	...
	if tree == null:
		return null
	_cached_object_pool = tree.root.get_node_or_null("ObjectPool")
	return _cached_object_pool
