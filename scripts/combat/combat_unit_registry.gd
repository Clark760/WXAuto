...
func add_unit_mid_battle(manager: CombatManager, unit: Node) -> bool:
...
func pre_tick_scan(manager: CombatManager) -> void:
...
func pre_tick_scan_incremental(manager: CombatManager) -> void:
...
func trim_component_caches(manager: CombatManager, valid_ids: Dictionary) -> void:
...
func trim_runtime_caches(manager: CombatManager, valid_ids: Dictionary) -> void:
...
func remove_unit_runtime_entry(manager: CombatManager, unit: Node) -> void:
	if not manager._is_live_unit(unit):
		return
...
func remove_runtime_entry_by_id(manager: CombatManager, iid: int) -> void:
...
func cache_components_for_unit(manager: CombatManager, unit: Node) -> void:
	if not manager._is_live_unit(unit):
		return
	var iid: int = unit.get_instance_id()
...
func bind_combat_component_signals(manager: CombatManager, unit: Node) -> void:
	if not manager._is_live_unit(unit):
		return
	var combat: Node = manager._get_combat(unit)
	if combat == null:
...
func unbind_combat_component_signals(manager: CombatManager, unit: Node) -> void:
	if not manager._is_live_unit(unit):
		return
	var combat: Node = manager._get_combat(unit)
	if combat == null:
...
func reset_tick_caches(manager: CombatManager) -> void:
	manager._alive_by_team[manager.TEAM_ALLY] = 0
	manager._alive_by_team[manager.TEAM_ENEMY] = 0
