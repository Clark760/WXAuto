...
const LOG_TYPE_DAMAGE: int = 0 # 普通伤害日志事件。
const LOG_TYPE_SKILL_DAMAGE: int = 1 # 技能伤害日志事件。
const LOG_TYPE_SKILL_HEAL: int = 2 # 技能治疗日志事件。
const LOG_TYPE_SKILL_CAST: int = 3 # 技能触发日志事件。
const LOG_TYPE_DEATH: int = 4 # 阵亡日志事件。
...
func _on_damage_resolved(event_dict: Dictionary) -> void:
	...
	_append_combat_event({
		"type": LOG_TYPE_DAMAGE,
		"source_name": _resolve_unit_name(
			source_unit,
			str(event_dict.get("source_name", "")),
			int(event_dict.get("source_id", -1))
		),
		"source_team": _resolve_unit_team(source_unit, int(event_dict.get("source_team", 0))),
		"source_id": _resolve_unit_instance_id(source_unit, int(event_dict.get("source_id", -1))),
		"target_name": _resolve_unit_name(
			target_unit,
			str(event_dict.get("target_name", "")),
			int(event_dict.get("target_id", -1))
		),
		"target_team": _resolve_unit_team(target_unit, int(event_dict.get("target_team", 0))),
		"target_id": _resolve_unit_instance_id(target_unit, int(event_dict.get("target_id", -1))),
		"value": int(round(float(event_dict.get("damage", 0.0)))),
		"timestamp": float(_state.combat_elapsed)
	})
...
func _on_skill_effect_damage(event_dict: Dictionary) -> void:
	...
	_append_combat_event({
		...
		"target_id": _resolve_unit_instance_id(target_unit, int(event_dict.get("target_id", -1))),
		"value": int(round(float(event_dict.get("damage", 0.0)))),
		"skill_name": _resolve_skill_name_from_event(event_dict),
		"timestamp": float(_state.combat_elapsed)
	})
...
# 技能触发事件用于补全“谁发动了什么技能”日志，不参与统计面板数值。
func _on_skill_triggered(unit: Node, gongfa_id: String, trigger: String) -> void:
	if not _capture_running:
		return
	var trigger_name: String = trigger.strip_edges().to_lower()
	if trigger_name == "passive" or trigger_name == "passive_aura":
		return
	var skill_name: String = _resolve_skill_name_from_gongfa_id(gongfa_id)
	_append_combat_event({
		"type": LOG_TYPE_SKILL_CAST,
		"source_name": _resolve_unit_name(unit, "", -1),
		"source_team": _resolve_unit_team(unit, 0),
		"source_id": _resolve_unit_instance_id(unit, -1),
...
func _on_unit_died(dead_unit: Node, killer: Node, _team_id: int) -> void:
	...
	_append_combat_event({
		"type": LOG_TYPE_DEATH,
		"source_name": _resolve_unit_name(killer, "", -1),
		"source_team": _resolve_unit_team(killer, 0),
		"source_id": _resolve_unit_instance_id(killer, -1),
		"target_name": _resolve_unit_name(dead_unit, "", -1),
		"target_team": _resolve_unit_team(dead_unit, 0),
		"target_id": _resolve_unit_instance_id(dead_unit, -1),
		"timestamp": float(_state.combat_elapsed)
	})
...
# 往 HUD 追加结构化战斗事件，交给 runtime_view 的聚合器统一节流渲染。
func _append_combat_event(event: Dictionary) -> void:
	if event.is_empty():
		return
	var presenter: Node = _get_hud_presenter()
	if presenter != null and presenter.has_method("append_combat_event"):
		presenter.append_combat_event(event)


func _resolve_unit_name(unit: Node, fallback_name: String = "", fallback_iid: int = -1) -> String:
	if is_instance_valid(unit):
		var from_node: String = str(unit.get("unit_name")).strip_edges()
		if not from_node.is_empty():
			return from_node
	var normalized_fallback: String = fallback_name.strip_edges()
	if not normalized_fallback.is_empty():
		return normalized_fallback
	if fallback_iid > 0:
		return "单位#%d" % fallback_iid
	return "未知单位"


func _resolve_unit_team(unit: Node, fallback_team: int = 0) -> int:
	if is_instance_valid(unit):
		return int(unit.get("team_id"))
	return fallback_team


func _resolve_unit_instance_id(unit: Node, fallback_iid: int = -1) -> int:
	if is_instance_valid(unit):
		return unit.get_instance_id()
	return fallback_iid


func _resolve_skill_name_from_event(event_dict: Dictionary) -> String:
	var explicit_name: String = str(event_dict.get("skill_name", "")).strip_edges()
	if not explicit_name.is_empty():
		return explicit_name
	var gongfa_id: String = str(event_dict.get("gongfa_id", "")).strip_edges()
	return _resolve_skill_name_from_gongfa_id(gongfa_id)


func _resolve_skill_name_from_gongfa_id(gongfa_id: String) -> String:
	var normalized_id: String = gongfa_id.strip_edges()
	if normalized_id.is_empty():
		return ""
	if _refs != null and _refs.unit_augment_manager != null:
		if _refs.unit_augment_manager.has_method("get_gongfa_data"):
			var gongfa_data: Dictionary = _refs.unit_augment_manager.get_gongfa_data(normalized_id)
			if not gongfa_data.is_empty():
				var gongfa_name: String = str(gongfa_data.get("name", normalized_id)).strip_edges()
				if not gongfa_name.is_empty():
					return gongfa_name
